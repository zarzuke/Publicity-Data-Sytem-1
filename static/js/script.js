...
function enviarValor(valor) {
    // Aquí puedes usar AJAX o Fetch para enviar el valor al servidor
    // Por ejemplo:
    fetch('/form', {
        method: 'POST',
        body: JSON.stringify({ 'job-type': valor }),
        headers: {
            'Content-Type': 'application/json'
        }
    })
    .then(response => response.text())
    .then(data => console.log(data))  // Maneja la respuesta del servidor
